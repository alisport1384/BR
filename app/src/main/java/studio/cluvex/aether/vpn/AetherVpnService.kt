 ...
 *      traffic through the proxy — replacing the need for legacy proxy entirely,
...
class AetherVpnService : VpnService() {
    ...
    private suspend fun connectAttempt(
        ...
        engine = AetherProcess(this).also { it.start(profile) }
        ...
        if (profile.proxyMode) {
            ...
            // Ports are FIXED (legacy proxy-style standard) — the same values are
    ...
    private fun establishTun(profile: ConnectionProfile) {
        ...
        // the TUN, equivalent to legacy proxy's in-process protect()).
    ...
    /**
     * Writes the hev-socks5-tunnel config in the exact shape legacy proxy uses.
     *
